...
class WallhavenApiV1:
    ...
    def search(self, q=None, categories=None, purities=None, sorting=None, order=None, top_range=None, atleast=None, 
        ...
        if categories is not None:
            ...
            params["categories"] = self._category(Category.general in categories,
        ...
        if purities is not None:
            ...
            params["purity"] = self._purity(Purity.sfw in purities,
        ...
        return self._request(method="get", url=self._url_format("search"), params=params)

    def wallpaper(self, wallpaper_id):
        return self._request(method="get", url=self._url_format("w", wallpaper_id))

    def is_walpaper_exists(self, wallpaper_id):
        return "error" not in self.wallpaper(wallpaper_id)

    def download_wallpaper(self, wallpaper_id, file_path=None, chunk_size=4096):
        wallpaper_data = self.wallpaper(wallpaper_id)

        if "error" in wallpaper_data:
            raise NoWallpaperError(wallpaper_id)

        wallpaper = requests.get(wallpaper_data["data"]["path"],
                                 stream=True,
    ...
    def tag(self, tag_id):
        return self._request(method="get", url=self._url_format("tag", tag_id))

    def settings(self):
        return None if self.api_key is None else self._request(method="get", url=self._url_format("settings"))

    def collections(self, user_name):
        return self._request(method="get", url=self._url_format(f"collections/{user_name}"))

    def collection_wallpapers(self, user_name, collection_id, page=None):
        return self._request(method="get", url=self._url_format(f"collections/{user_name}/{collection_id}"), 
            params={"page": str(page)} if page is not None else {})

    def my_collections(self):
        return None if self.api_key is None else self._request(method="get", url=self._url_format(f"collections"))